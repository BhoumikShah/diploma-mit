...
// Function to do inorder traversal and find the node with the specified data
void inorderSearch(struct Node* root, struct Index* idx, int targetData) {
    if (root == NULL) {
        return;
    }

    inorderSearch(root->left, idx, targetData);

    idx->count++;
    if (root->data == targetData && idx->index == -1) {
        idx->index = idx->count;
    }

    inorderSearch(root->right, idx, targetData);
}

// Function to search for a node with a specific data value in the binary search tree
